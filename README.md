...
- I have a personal rule where I try not to commit potentially sensitive stuff, though I fully realize that creating an environment variable for the Django SECRET KEY is probably overkill for this scenario.
- I will send the key to Maria via email once I turn in the repo. :)
...
2. **Running included bash script with seed that database twice.**
- So don't do that :)
- But if you do do that, just delete all the instances and run the script again:
```python
./manage.py shell
